...
export const create = ({ bodymen: { body: { email, link } } }, res, next) =>
  User.findOne({ email })
    ...
    .then((reset) => {
      ...
      const content = `
        Hey, ${user.name}.<br><br>
        You requested a new password for your IAmDB-server account.<br>
        ...
        &mdash; IAmDB-server Team
      `
      return sendMail({ toEmail: email, subject: 'IAmDB-server - Password Reset', content })
    })
